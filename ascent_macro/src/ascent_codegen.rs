...
use crate::ascent_mir::{
    ir_relation_version_var_name, mir_rule_summary, mir_summary, AscentMir, MirBodyItem,
    MirRelation, MirRelationVersion, MirRule, MirScc,
};
use crate::utils::{exp_cloned, expr_to_ident, tuple, tuple_spanned, tuple_type};
use crate::{
    ascent_hir::{IndexValType, IrRelation},
    ascent_syntax::{CondClause, RelationIdentity},
    utils::TokenStreamExtensions,
};
...
pub(crate) fn compile_mir(mir: &AscentMir, is_ascent_run: bool) -> proc_macro2::TokenStream {
    ...
    let sorted_relations_ir_relations = mir
        .relations_ir_relations
        .iter()
        .sorted_by_key(|(rel, _)| &rel.name);
    for (rel, rel_indices) in sorted_relations_ir_relations {
        let name = &rel.name;
        let rel_attrs = &mir.relations_metadata[rel].attributes;
        let sorted_rel_index_names = rel_indices
            .iter()
            .map(|ind| format!("{}", ind.ir_name()))
            .sorted();
        ...
        field_defaults
            .push(quote! {#name : Default::default(), #rel_ind_common: Default::default(),});
    ...
    let (ty_impl_generics, ty_ty_generics, ty_where_clause) =
        mir.signatures.split_ty_generics_for_impl();
    let (impl_impl_generics, impl_ty_generics, impl_where_clause) =
        mir.signatures.split_impl_generics_for_impl();

    let ty_signature = &mir.signatures.declaration;
    if let Some(impl_signature) = &mir.signatures.implementation {
        assert_eq!(
            ty_signature.ident, impl_signature.ident,
            "The identifiers of struct and impl must match"
        );
    }
    ...
    for rel in mir.relations_ir_relations.keys() {
        ...
        rel_codegens
            .push(quote_spanned! { macro_path.span()=> #macro_path::rel_codegen!{#macro_input} });
    }
...
fn rel_index_type(rel: &IrRelation, mir: &AscentMir) -> Type {
    ...
    let is_lat_full_index =
        rel.relation.is_lattice && &mir.lattices_full_indices[&rel.relation] == rel;
...
fn rel_ds_macro_input(rel: &RelationIdentity, mir: &AscentMir) -> TokenStream {
    ...
    let name = Ident::new(
        &format!("{}_{}", mir.signatures.declaration.ident, rel.name),
        span,
    );
    quote! {
       #name,
...
fn rule_time_field_name(scc_ind: usize, rule_ind: usize) -> Ident {
    Ident::new(
        &format!("rule{}_{}_duration", scc_ind, rule_ind),
        Span::call_site(),
    )
}
...
fn compile_mir_scc(mir: &AscentMir, scc_ind: usize) -> proc_macro2::TokenStream {
    ...
    let sorted_body_only_relations = scc
        .body_only_relations
        .iter()
        .sorted_by_cached_key(|(rel, _)| rel.name.clone());
...
fn compile_mir_rule_inner(
    ...
    _scc: &MirScc,
    mir: &AscentMir,
    par_iter_to_ind: usize,
    head_update_code: proc_macro2::TokenStream,
    clause_ind: usize,
) -> proc_macro2::TokenStream {
    if Some(clause_ind) == rule.simple_join_start_index && rule.reorderable {
        ...
        let rule_cp1_compiled = compile_mir_rule_inner(
            &rule_cp1,
            _scc,
            mir,
            par_iter_to_ind,
            head_update_code.clone(),
            clause_ind,
        );
        let rule_cp2_compiled = compile_mir_rule_inner(
            &rule_cp2,
            _scc,
            mir,
            par_iter_to_ind,
        ...
        if let [MirBodyItem::Clause(bcl1), MirBodyItem::Clause(bcl2)] =
            &rule.body_items[clause_ind..clause_ind + 2]
        {
        ...
        match bitem {
            MirBodyItem::Clause(bclause) => {
                let (clause_ind, bclause) = if doing_simple_join {
                    (
                        clause_ind + 1,
                        rule.body_items[clause_ind + 1].unwrap_clause(),
                    )
                } else {
                    (clause_ind, bclause)
                };
                ...
                let common_vars = clause_vars
                    .iter()
                    .filter(|(_i, v)| pre_clause_vars.contains(v))
                    .collect::<Vec<_>>();
                let common_vars_no_indices = common_vars
                    .iter()
                    .map(|(_i, v)| v.clone())
                    .collect::<Vec<_>>();
                ...
                let new_vars_assignments = clause_var_assignments(
                    ...
                    clause_vars
                        .iter()
                        .filter(|(_i, var)| !common_vars_no_indices.contains(var))
                        .cloned(),
                    &matched_val_ident,
                ...
                let (index_get, iter_all) = if clause_ind < par_iter_to_ind {
                    (
                        quote_spanned! {span=> c_index_get},
                        quote_spanned! {span=> c_iter_all},
                    )
                } else {
                    (
                        quote_spanned! {span=> index_get},
                        quote_spanned! {span=> iter_all},
                    )
                };
                ...
                if doing_simple_join {
                    let cl1 =
                        rule.body_items[rule.simple_join_start_index.unwrap()].unwrap_clause();
                    let cl2 = bclause;
                    ...
                    let cl1_vars_assignments = clause_var_assignments(
                        ...
                        cl1_vars
                            .iter()
                            .filter(|(i, _var)| !cl1.rel.indices.contains(i))
                            .cloned(),
                    ...
                    let joined_args_for_cl2_cloned =
                        cl2.selected_args().iter().map(exp_cloned).collect_vec();
                    let joined_args_tuple_for_cl2 =
                        tuple_spanned(&joined_args_for_cl2_cloned, cl2.args_span);

                    let cl1_tuple_indices_iter =
                        quote_spanned!(cl1.rel_args_span=> __cl1_tuple_indices);
            ...
            MirBodyItem::Agg(agg) => {
                ...
                let agg_args_tuple_indices = agg.bound_args.iter().map(|arg| {
                    (
                        agg.rel_args
                            .iter()
                            .find_position(|rel_arg| expr_to_ident(rel_arg) == Some(arg.clone()))
                            .unwrap()
                            .0,
                        arg.clone(),
                    )
                ...
                let agg_args_tuple = tuple_spanned(
                    &agg.bound_args
                        .iter()
                        .map(|v| parse_quote! {#v})
                        .collect_vec(),
...
fn head_clauses_structs_and_update_code(
    ...
    for hcl in rule.head_clauses.iter() {
        ...
        let (rel_index_write_trait, index_insert_fn) = (
            rel_index_write_trait.with_span(hcl.span),
            index_insert_fn.with_span(hcl.span),
        );
        ...
        let expr_for_rel_maybe_mut = if mir.is_parallel {
            expr_for_c_rel_write
        } else {
            expr_for_rel_write
        };
        ...
        let head_rel_full_index_expr_delta =
            expr_for_rel(&MirRelation::from(head_rel_full_index.clone(), Delta), mir);
        let head_rel_full_index_expr_total =
            expr_for_rel(&MirRelation::from(head_rel_full_index.clone(), Total), mir);
        ...
        if !hcl.rel.is_lattice {
            ...
            let head_lat_full_index_var_name_new =
                ir_relation_version_var_name(&head_lat_full_index.ir_name(), New);
            ...
            let head_lat_full_index_var_name_full =
                ir_relation_version_var_name(&head_lat_full_index.ir_name(), Total);
...
fn lattice_insertion_mutex_var_name(head_relation: &RelationIdentity) -> Ident {
    Ident::new(
        &format!("__{}_mutex", head_relation.name),
        head_relation.name.span(),
    )
}

fn rel_ind_common_var_name(relation: &RelationIdentity) -> Ident {
    Ident::new(
        &format!("__{}_ind_common", relation.name),
        relation.name.span(),
    )
}
...
fn expr_for_rel(rel: &MirRelation, mir: &AscentMir) -> proc_macro2::TokenStream {
    fn expr_for_rel_inner(
        ...
        if mir_rel.relation.is_lattice {
            ...
            let rel_ind_common =
                ir_relation_version_var_name(&rel_ind_common_var_name(&mir_rel.relation), version);
...
fn expr_for_rel_write(mir_rel: &MirRelation, _mir: &AscentMir) -> proc_macro2::TokenStream {
    ...
    if mir_rel.relation.is_lattice {
        ...
        let rel_ind_common = ir_relation_version_var_name(
            &rel_ind_common_var_name(&mir_rel.relation),
            mir_rel.version,
        );
...
fn index_get_entry_val_for_insert(
    rel_ind: &IrRelation,
    tuple_expr: &Expr,
    ind_expr: &Expr,
) -> Expr {
