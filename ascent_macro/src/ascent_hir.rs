...
use crate::{
    ...
    syn_utils::{expr_get_vars, pattern_get_vars},
    utils::{expr_to_ident, is_wild_card, tuple_type},
    AscentProgram,
};
...
impl AscentConfig {
    ...
    pub fn new(attrs: Vec<Attribute>, is_parallel: bool) -> syn::Result<AscentConfig> {
        ...
        for attr in attrs.iter() {
            if !recognized_attrs
                .iter()
                .any(|recognized_attr| attr.meta.path().is_ident(recognized_attr))
            {
                return Err(Error::new_spanned(
                    attr,
                    format!(
                        "unrecognized attribute. recognized attributes are: {}",
        ...
        if inter_rule_parallelism.is_some() && !is_parallel {
            return Err(Error::new_spanned(
                ...
                "attribute only allowed in parallel Ascent",
            ));
        }
        let default_ds = get_ds_attr(&attrs)?.unwrap_or_else(|| DsAttributeContents {
            path: parse_quote! {::ascent::rel},
            args: TokenStream::default(),
        });
        Ok(AscentConfig {
...
impl IrRelation {
    ...
    pub fn key_type(&self) -> Type {
        let index_types: Vec<_> = self
            .indices
            .iter()
            .map(|&i| self.relation.field_types[i].clone())
            .collect();
...
pub(crate) fn compile_ascent_program_to_hir(
    prog: &AscentProgram,
    is_parallel: bool,
) -> syn::Result<AscentIr> {
    ...
    for rel in prog.relations.iter() {
        ...
        if rel.is_lattice {
            ...
            relations_ir_relations
                .entry(rel_identity.clone())
                .or_default()
                .insert(lat_full_index.clone());
            ...
            RelationMetadata {
                ...
                attributes: Rc::new(
                    rel.attrs
                        .iter()
                        .filter(|attr| {
                            attr.meta.path().get_ident().map_or(true, |ident| {
                                !RECOGNIIZED_REL_ATTRS.iter().any(|ra| ident == ra)
                            })
                        })
                        .cloned()
                        .collect_vec(),
    ...
    for (ir_rule, extra_relations) in ir_rules.iter() {
        ...
        for extra_rel in extra_relations.iter() {
            relations_ir_relations
                .entry(extra_rel.relation.clone())
                .or_default()
                .insert(extra_rel.clone());
        }
    }
    let signatures = prog
        .signatures
        .clone()
        .unwrap_or_else(|| parse2(quote! {pub struct AscentProgram;}).unwrap());
    Ok(AscentIr {
        rules: ir_rules
            .into_iter()
            .map(|(rule, _extra_rels)| rule)
            .collect_vec(),
        relations_ir_relations,
    ...
    let ds_attrs = attrs
        ...
        .filter(|attr| {
            attr.meta
                .path()
                .get_ident()
                .map_or(false, |ident| ident == REL_DS_ATTR)
        })
        .collect_vec();
    match &ds_attrs[..] {
        [] => Ok(None),
        ...
        [_attr1, attr2, ..] => Err(Error::new(
            attr2.bracket_token.span.join(),
            "multiple `ds` attributes specified",
        )),
    }
}
...
fn compile_rule_to_ir_rule(
    ...
    fn extend_grounded_vars(
        grounded_vars: &mut Vec<Ident>,
        new_vars: impl IntoIterator<Item = Ident>,
    ) -> syn::Result<()> {
        for v in new_vars.into_iter() {
            if grounded_vars.contains(&v) {
                ...
                let mut err = Error::new(
                    v.span(),
                    format!("'{}' shadows another variable with the same name", v),
                );
    ...
    let first_clause_ind = rule
        ...
        .iter()
        .enumerate()
        .find(|(_, bi)| matches!(bi, BodyItemNode::Clause(..)))
        .map(|(i, _)| i);
    let mut first_two_clauses_simple = first_clause_ind.is_some()
        && matches!(
            rule.body_items.get(first_clause_ind.unwrap() + 1),
            Some(BodyItemNode::Clause(..))
        );
            ...
            BodyItemNode::Clause(ref bcl) => {
                if first_clause_ind == Some(bitem_ind)
                    && bcl
                        .cond_clauses
                        .iter()
                        .any(|c| matches!(c, &CondClause::IfLet(_)))
                {
                ...
                if first_clause_ind.map(|x| x + 1) == Some(bitem_ind) && first_two_clauses_simple {
                    ...
                    for var in bcl
                        .args
                        .iter()
                        .filter_map(|arg| expr_to_ident(arg.unwrap_expr_ref()))
                    {
                ...
                let ir_bcl = IrBodyClause {
                    ...
                    args: bcl
                        ...
                        .iter()
                        .cloned()
                        .map(BodyClauseArg::unwrap_expr)
                        .collect(),
                    rel_args_span: bcl
                        .rel
                        .span()
                        .join(bcl.args.span())
                        .unwrap_or_else(|| bcl.rel.span()),
        ...
        let rel = match rel {
            ...
            None => {
                return Err(Error::new(
                    hcl_node.rel.span(),
                    format!("relation {} not defined", hcl_node.rel),
                ))
            }
        };
    ...
    let simple_join_start_index = if is_simple_join {
        first_clause_ind
    } else {
        None
    };
...
pub(crate) fn prog_get_relation<'a>(
    ...
    match relation {
        Some(rel) => {
            if rel.field_types.len() != arity {
                Err(Error::new(
                    name.span(),
                    format!(
                        "Wrong arity for relation {}. Actual arity: {}",
                        name,
                        rel.field_types.len()
                    ),
                ))
        ...
        None => Err(Error::new(
            name.span(),
            format!("Relation {} not defined", name),
        )),
    }
}
