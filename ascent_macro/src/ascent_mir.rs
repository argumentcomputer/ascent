...
pub(crate) fn mir_summary(mir: &AscentMir) -> String {
    ...
    for (i, scc) in mir.sccs.iter().enumerate() {
        ...
        let sorted_dynamic_relation_keys =
            scc.dynamic_relations.keys().sorted_by_key(|rel| &rel.name);
        write!(&mut res, "  dynamic relations: ").unwrap();
        writeln!(
            &mut res,
            "{}",
            sorted_dynamic_relation_keys
...
pub(crate) fn mir_rule_summary(rule: &MirRule) -> String {
    ...
    format!(
        ...
        simple_join = if rule.simple_join_start_index.is_some() {
            " [SIMPLE JOIN]"
        } else {
            ""
        },
        reorderable = if rule.simple_join_start_index.is_some() && !rule.reorderable {
            " [NOT REORDERABLE]"
        } else {
            ""
        }
...
pub(crate) fn compile_hir_to_mir(hir: &AscentIr) -> syn::Result<AscentMir> {
    ...
    for scc in sccs.node_weights().collect_vec().iter().rev() {
        ...
        let mut body_only_relations: HashMap<RelationIdentity, HashSet<IrRelation>> =
            HashMap::new();
    ...
    for n in sccs.node_indices() {
        ...
        sccs_dep_graph.insert(
            ...
            sccs.neighbors(n)
                .map(|n| sccs_nodes_count - n.index() - 1)
                .collect(),
        );
    }
...
fn compile_hir_rule_to_mir_rules(
    rule: &IrRule,
    dynamic_relations: &HashSet<RelationIdentity>,
) -> Vec<MirRule> {
    ...
    fn versions(
        dynamic_cls: &[usize],
        simple_join_start_index: Option<usize>,
    ) -> Vec<Vec<MirRelationVersion>> {
    ...
    fn hir_body_item_to_mir_body_item(
        hir_bitem: &IrBodyItem,
        version: Option<MirRelationVersion>,
    ) -> MirBodyItem {
        match hir_bitem {
            IrBodyItem::Clause(_) => {}
    ...
    for version_combination in version_combinations {
        let versions = dynamic_cls.iter().zip(version_combination).fold(
            vec![None; rule.body_items.len()],
            |mut acc, (i, v)| {
                acc[*i] = Some(v);
                acc
            },
        ...
        let mir_bodys = rule
            ...
            .iter()
            .zip(versions)
            .map(|(bi, v)| hir_body_item_to_mir_body_item(bi, v))
            .collect_vec();
