    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        while !input.is_empty() {
            ...
            if input.peek(kw::relation) || input.peek(kw::lattice) {
                ...
                if !attrs.is_empty() {
                    return Err(Error::new(attrs[0].span(), "unexpected attribute(s) 1"));
                }
...
fn body_item_visit_exprs_free_vars_mut(
    ...
    match bi {
        ...
        BodyItemNode::MacroInvocation(m) => {
            update(&mut m.mac.tokens, |ts| {
                token_stream_replace_ident(ts, visitor)
            });
        }
    }
}
...
fn rule_expand_macro_invocations(
    ...
    fn body_item_expand_macros(
        ...
        match bi {
            ...
            BodyItemNode::Disjunction(disj) => {
                ...
                Ok(punctuated_singleton(BodyItemNode::Disjunction(
                    DisjunctionNode {
    ...
    let new_head_items = punctuated_map(rule.head_clauses, |hi| {
        head_item_expand_macros(hi, macros, 100, None)
    })
    .pipe(punctuated_try_unwrap)?
    .pipe(flatten_punctuated);
